...
try {
  ...
  if (!Array.isArray(graph.layers)) { warnings.push('graph.layers is not an array'); graph.layers = []; }
  if (!Array.isArray(graph.tour)) { warnings.push('graph.tour is not an array'); graph.tour = []; }
  ...
  graph.tour.forEach((step, i) => (step.nodeIds || []).forEach(id => {
