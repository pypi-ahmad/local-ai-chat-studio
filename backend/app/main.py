...
def create_app(
    ...
    async def session_cookie(request: Request, call_next):
        ...
        if "chat_session" not in request.cookies:
            response.set_cookie(
                "chat_session", session_id, httponly=True, samesite="strict", secure=False
            )
        return response
            ...
            "providers": [
                {
                    "id": provider,
                    "label": label,
                    "key_source": vault.source(request.state.session_id, provider),
                }
    ...
    def start_openrouter_auth(request: Request) -> dict[str, str]:
        ...
        oauth_verifiers[request.state.session_id] = verifier
    ...
    def set_credential(provider: str, payload: CredentialInput, request: Request) -> Response:
        ...
        vault.set(request.state.session_id, provider, payload.api_key.strip())
