...
**CI workflow:** `.github/workflows/ci.yml` triggers on every `push` and `pull_request`. Three parallel jobs run: `launcher-windows` (windows-latest), `backend` (ubuntu-latest), `frontend` (ubuntu-latest).

**CI enforcement:** [UNVERIFIED]. Whether branch-protection rules make these jobs *required* status checks cannot be determined from the local checkout. As of this writing the CI merely runs on all branches/PRs; whether merges are blocked on failure is a manual repository-settings decision.
...
1. **`launcher-windows`** (`windows-latest`): runs `Launch Chat Studio.cmd --check`, which validates the Windows launcher syntax and setup flow.
...
**Rule:** `backend/app/` modules never import from `frontend/`. `src/` modules never import from `backend/app/`. `frontend/src/api/schema.ts` is generated, so never edit it by hand.
...
**Consequences:** Simplicity, with no extra infrastructure needed. Concurrent run count is limited by the event loop and Python's GIL for CPU-bound work, and a restart clears all in-flight runs.
...
- `create_tool_request`: validates arguments against the tool's JSON Schema (`Draft202012Validator`) before storing, so malformed arguments are rejected at submission, not at execution (`main.py:269-270`).
