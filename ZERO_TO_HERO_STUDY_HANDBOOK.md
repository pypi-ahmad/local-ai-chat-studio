...
`frontend/src/App.tsx` is the live workspace: Chat, Compare, Providers, Library, Activity, and Settings. It preflights turns, streams SSE run events, manages credentials, displays source-linked model rates and estimated input cost, and calls `api.shutdown()` for **Stop Studio**.
