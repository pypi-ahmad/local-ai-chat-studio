...
**Token.** The unit a model actually reads/writes, often a word-piece rather
than a whole word. `"unhappiness"` might be three tokens. Cost, speed, and the
...
**Temperature.** A sampling knob, roughly 0 to 2. Low = more deterministic/
...
**Cosine similarity.** The standard way to compare two embedding vectors. It
measures the angle between them, ignoring length. Range is theoretically
...
times in this repo (documents, cross-chat memory, long-term facts): the same
technique, applied to three different collections.
...
event loop, so no separate OS thread is needed, just cooperative scheduling.
Same underlying goal (don't block the user while a model streams), two
different tools because the two web frameworks work fundamentally differently.
...
pattern: one import, one source of truth that's trivially overridable at
deploy time.
...
conversation using." It's a plain string like `ollama::llama3.2` or
...
a lock (because a background thread reads it too, see 3.10), with an
environment-variable fallback. There is **no database table for API keys**,
and that's not an oversight. It's the whole privacy promise: keys never
touch disk, so there's nothing to leak from disk. Restart the app, the dict
is empty again.
...
### 3.10 `src/jobs.py`: the background worker
...
(every 8 messages) extracting new memories. Each is wrapped so a failure
there never breaks the actual reply the user already has on screen:
...
That's the entire legacy request lifecycle: UI writes immediately, hands off
to a thread, polls a shared dict, thread persists on completion, UI reads
the persisted truth.
...
Unlike Streamlit (one server process per user, typically), a FastAPI server
can serve multiple browsers at once, so credentials must be partitioned per
session. This `session_id` comes from a cookie set by `main.py`'s
middleware, which brings us to the file where this session's own real bug
lived.
...
> A real bug lived right next to this code. `asyncio.create_task(...)`'s
> return value is discarded on line 41 above; nothing keeps a strong
> reference to that task object. Per the
...
> The rule: **if you don't keep the task object, Python is allowed to throw
> it away.**
...
Now the session cookie middleware. Read carefully: this is where a real,
verified bug lived in this exact codebase until this session.
...
That `samesite="lax"` used to read `samesite="strict"`. That one word broke
a whole feature, for the exact reason the PKCE theory from §3.5
([PKCE](https://www.rfc-editor.org/rfc/rfc7636)) predicts:
...
guaranteeing a `409` on every single real attempt. `SameSite=Lax` is the
...
Both traces follow the same shape: save fast, hand off the work, stream
progress, persist on completion. They're implemented with completely
...
- Use the app to build intuition first, then re-read whichever section above
  maps to the feature you just used. [USER_GUIDE.md](USER_GUIDE.md) covers
  every feature from the user's side.
- For reference tables, glossary, module atlas, and contributor recipes, see
  [ZERO_TO_HERO_STUDY_HANDBOOK.md](ZERO_TO_HERO_STUDY_HANDBOOK.md).
- Try the exercises the handbook already has (§13). Tracing a run with the
  `echo` provider with no real model needed is a good first one.
...
- A good first contribution: a focused UI or contract change with a matching
  test. See [CONTRIBUTING.md](CONTRIBUTING.md). The old "wire the static
  shell" gap list is obsolete as of v0.4.0.
