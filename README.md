...
- **Conversation management:** Create, search, rename, pin, file into durable folders, and delete conversations. The adjustable desktop history groups pinned chats, folders, and unfiled chats by date; search matches titles and folders. Branch from any message to explore another direction while preserving the earlier history.
...
- **Focused navigation and commands:** Desktop keeps Chat, Compare, and Library in **Primary**, Focus and Tools in **Workspace**, and provider/runtime controls in **Administration**. Mobile keeps the three primary destinations one tap away and moves advanced workspaces into **More**. Press `Ctrl+K` on Windows/Linux or `Cmd+K` on macOS to search workspace and run commands.
...
- **Responsive workspace feedback:** Conversation and workspace skeletons replace blank initial loads, empty chats offer useful starter prompts, and stored tool results stay collapsed until expanded. The sticky composer and attachment cards remain visible at the bottom of the working area.
